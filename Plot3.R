...
##Calculate pollution for each year
Total.Pollution <- ddply(Balt, .(year, type), function(x) data.frame(Emissions = sum(x$Emissions)))

#   ## Could also use
#   Total.Pollution <- aggregate(Emissions ~ type + year, data = Balt, sum)
...
with(Total.Pollution, ggplot(Total.Pollution, aes(year, Emissions)) + geom_point(aes(color = type)) + 
